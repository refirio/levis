...
function file_mimetype($file)
{
    ...
    if (isset($info['extension']) && isset($mime_content_types[$info['extension']])) {
...
function file_getcsv($handle, $length = null, $delimiter = ',', $escape = '"')
{
    ...
    $csv_pattern = '/(' . $escape . '[^' . $escape . ']*(?:' . $escape . $escape . '[^' . $escape . ']*)*' . $escape . '|[^' . $delimiter . ']*)' . $delimiter . '/';

    if (preg_match_all($csv_pattern, $csv_line, $csv_matches)) {
